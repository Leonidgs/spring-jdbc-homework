package ru.diasoft.spring.controller;

import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.http.MediaType;
import org.springframework.security.test.context.support.WithMockUser;
import org.springframework.test.context.ActiveProfiles;
import org.springframework.test.web.servlet.MockMvc;

import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.delete;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.put;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class CommentControllerSecurityTest {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void getCommentsByBook_unauthenticated_returnsRedirect() throws Exception {
        mockMvc.perform(get("/api/books/1/comments"))
                .andExpect(status().is3xxRedirection());
    }

    @Test
    void createComment_unauthenticated_returnsRedirect() throws Exception {
        mockMvc.perform(post("/api/books/1/comments")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"text\":\"Test\"}"))
                .andExpect(status().is3xxRedirection());
    }

    @Test
    void getCommentById_unauthenticated_returnsRedirect() throws Exception {
        mockMvc.perform(get("/api/comments/1"))
                .andExpect(status().is3xxRedirection());
    }

    @Test
    void updateComment_unauthenticated_returnsRedirect() throws Exception {
        mockMvc.perform(put("/api/comments/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"text\":\"Test\"}"))
                .andExpect(status().is3xxRedirection());
    }

    @Test
    void deleteComment_unauthenticated_returnsRedirect() throws Exception {
        mockMvc.perform(delete("/api/comments/1"))
                .andExpect(status().is3xxRedirection());
    }

    @Test
    @WithMockUser(username = "user")
    void getCommentsByBook_authenticated_returnsOk() throws Exception {
        mockMvc.perform(get("/api/books/1/comments"))
                .andExpect(status().isOk());
    }

    @Test
    @WithMockUser(username = "user")
    void getCommentById_authenticated_returnsOk() throws Exception {
        mockMvc.perform(get("/api/comments/1"))
                .andExpect(status().isOk());
    }
}
